...
from gurupod.models.responses import EpisodeResponse, EpisodeResponseNoDB, EpisodeWithGurus
...
def test_import_new_episodes(random_episode_json, test_db):
    ...
    episode = EpisodeWithGurus.model_validate(response_data["episodes"][0])
    assert isinstance(episode, EpisodeWithGurus)
...
def test_fetch_new_episode(test_db):
    ...
    assert isinstance(data.episodes[0], EpisodeWithGurus)
...
def test_read_one_episode(random_episode_json, test_db):
    ...
    assert isinstance(res.episodes[0], EpisodeWithGurus)
