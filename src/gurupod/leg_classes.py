...
from gurupod.consts import MAIN_URL, REDDIT_CLIENT_ID, REDDIT_CLIENT_SEC, REDDIT_REF_TOK, REDIRECT, \
    USER_AGENT
...
#
#
# def create_markup(format, input_dict):
#     '''
...
#     markup_text = ""
#
#     for episode_name, details in input_dict.items():
...
class Pod:
    ...
    def __post_init__(self):
        if self.json_input:
            with open(self.json_input, 'r') as input_json:
    ...
    def update_json(self):
        ...
        for episode in self.new_episodes:
            update_package[episode.show_name] = {
                'show_url': episode.episode_url,
    ...
    def write_markup(self, format):
        ...
        if format.lower() == "reddit":
            with open('data/episodes.txt', "w", encoding="utf-8") as output_txt:
                markdown_text = create_markup(format)
            ...
            with open('data/episodes.html', "w", encoding="utf-8") as output_html:
                markup_text = create_markup(format)
...
class Episode:
    ...
    def from_url(cls, episode_url):
        ...
        show_name: str = episode_soup.select_one(".episode-title").text
        show_links: dict = get_show_links()
        show_notes: list = get_show_notes()
        show_date = parse(episode_soup.select_one(".publish-date").text)
        print(f"Episode object Created from Url: {show_name}")
    ...
    def episode_from_url(self):
        ...
        self.show_notes: list = get_show_notes()
